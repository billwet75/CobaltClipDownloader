...
- прямая загрузка видео и клипов VK (vk.com, vk.ru, vkvideo.ru) через VK API
  при указанном в настройках токене, включая приватные видео аккаунта;
...
1. Во вкладке **Настройки** введите VK access token в поле
   «VK access token (для приватных видео)» и сохраните.
2. При наличии токена ссылки VK (vk.com, vk.ru, vkvideo.ru, включая клипы)
   обрабатываются напрямую через VK API `video.get`, поэтому доступны и
   приватные видео, которые видит аккаунт токена.
3. Выбранное в настройках качество применяется и к VK: скачивается
   максимальный вариант не выше выбранного.
4. Без токена ссылки VK отправляются в cobalt, как и остальные сервисы.
5. В аудио-режиме ссылки VK всегда обрабатываются через cobalt, который умеет
   извлекать звук; прямой путь через VK API отдаёт только видео.
